...
| RF-M05 | Confirmar y concretar la compra de tickets de forma in-app, soportando los métodos de pago **Tarjeta** (con validación Regex de número, titular, vencimiento y CVV) y **Transferencia bancaria** (muestra datos CBU/Alias). El `PurchaseViewModel` aplica validaciones locales antes de enviar la solicitud a la API. | Alta |
| RF-M06 | Consultar el historial de tickets adquiridos por el usuario activo con soporte Offline. | Media |
| RF-M07 | Mostrar mensajes de error específicos al usuario cuando la API rechaza el pago (HTTP 400), extrayendo el campo `"mensaje"` del cuerpo JSON de la respuesta. | Alta |
...
## 4.3 Flujo de Compra con Método de Pago (`PurchaseScreen` + `PurchaseViewModel`)
1. `PurchaseScreen` renderiza dos `RadioButton` para seleccionar entre **Tarjeta** y **Transferencia**.
2. Al cambiar la selección, `PurchaseViewModel.onMetodoPagoChanged()` actualiza el estado. La UI muestra los campos de tarjeta condicionalmente (solo si el método es `"Tarjeta"`) o la `Card` informativa con CBU/Alias (si es `"Transferencia"`).
3. Al presionar "Confirmar", `PurchaseViewModel.validarYContinuar()` aplica las siguientes validaciones con Regex (idénticas a las del cliente Web):
   - Número: `^\d{16}$` — exactamente 16 dígitos.
   - Titular: `^[A-Za-zÁÉÍÓÚáéíóúÑñ\s]+$` — solo letras y espacios.
   - Vencimiento: `^(0[1-9]|1[0-2])/\d{2}$` + fecha futura (calculada con `Calendar`).
   - CVV: `^\d{3,4}$` — 3 o 4 dígitos.
4. Si el método es `"Transferencia"`, se omiten las validaciones de tarjeta y se avanza directamente.
5. Si la API responde con `HTTP 400` (`retrofit2.HttpException`), el `PaymentViewModel` extrae el campo `"mensaje"` del `errorBody()` y actualiza el estado a `PaymentUiState.Rejected(mensaje)`, que la `ValidationScreen` presenta al usuario.
...
| POST | `/api/Compra/crear` | `CompraResponseDto` (Ticket Confirmado) **o** `HTTP 400 { "mensaje": "..." }` (pago rechazado) | Sí |
...
- `CrearPagoDto`: MetodoPago, Monto, NumeroTarjeta?, Titular?, Vencimiento?, Cvv?. *(El campo `EstadoPago` fue eliminado; el backend lo determina internamente.)*
- `CrearTicketDto`: PartidoId, Sector, Fila, Asiento, Precio.

### Modelo de Dominio `Pago.kt`
```kotlin
data class Pago(
    val metodoPago:    String,           // "Tarjeta" | "Transferencia"
    val monto:         Double,
    val numeroTarjeta: String? = null,   // null si Transferencia
